...
use plonky2::plonk::config::{GenericConfig, ProverCompute};
use plonky2::timed;
...
macro_rules! impl_stark_prove {
    ($($async_kw:tt)*) => {
        /// From a STARK trace, computes a STARK proof to attest its correctness.
        pub $($async_kw)* fn prove<F, C, S, const D: usize>(
            stark: S,
            config: &StarkConfig,
            ...
            S: Stark<F, D>,
        {
            ...
            let trace_commitment = timed!(
                ...
                plonky2::maybe_await!(C::Compute::compute_from_values(
                    timing,
            ...
            plonky2::maybe_await!(prove_with_commitment(
                &stark,
                config,
                &trace_poly_values,
                &trace_commitment,
...
impl_stark_prove!(async);

macro_rules! impl_stark_prove_with_commitment {
    ($($async_kw:tt)*) => {
        ...
        pub $($async_kw)* fn prove_with_commitment<F, C, S, const D: usize>(
            stark: &S,
            config: &StarkConfig,
            trace_poly_values: &[PolynomialValues<F>],
            trace_commitment: &PolynomialBatch<F, C, D>,
            ctl_data: Option<&CtlData<'_, F>>,
            ...
            S: Stark<F, D>,
        {
            ...
            let num_ctl_polys: Vec<usize> = ctl_data
            ...
            let auxiliary_polys_commitment = if let Some(aux_polys) = auxiliary_polys {
                timed!(
                    ...
                    anyhow::Ok(Some(plonky2::maybe_await!(C::Compute::compute_from_values(
                        timing,
                        aux_polys,
                        rate_bits,
                        false,
            ...
            let (quotient_commitment, quotient_polys_cap) = if let Some(quotient_polys) = quotient_polys {
                ...
                let quotient_commitment = timed!(
                    ...
                    plonky2::maybe_await!(C::Compute::compute_from_coeffs(
                        timing,
                        all_quotient_chunks,
                        config.fri_config.cap_height,
                        rate_bits,
...
#[cfg(not(feature = "async_prover"))]
impl_stark_prove_with_commitment!();

#[cfg(feature = "async_prover")]
impl_stark_prove_with_commitment!(async);
