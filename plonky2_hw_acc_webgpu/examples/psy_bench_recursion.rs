    ...
    DummyPsyTypeCCircuit<F, C, D> where C::Hasher: AlgebraicHasher<F>
{
    pub fn new(config: &CircuitConfig) -> Self {
        ...
        DummyPsyTypeCCircuit { dummy_public_inputs, circuit_data }
    }
    pub fn prove(&self, dummy_public_inputs: HashOut<F>) -> Result<ProofWithPublicInputs<F, C, D>> {
        ...
        println!("Starting to prove dummy psy type c (WebGPU)...");
    ...
    DummyPsyTypeCRecursiveVerifierCircuit<F, C, D> where C::Hasher: AlgebraicHasher<F>
{
    pub fn new(config: &CircuitConfig, child_cap_height: usize, child_common_data: &CommonCircuitData<F, D>) -> Self {
        ...
        builder.verify_proof::<C>(&proof_a_target, &proof_a_verifier_data_target, child_common_data);
        builder.verify_proof::<C>(&proof_b_target, &proof_b_verifier_data_target, child_common_data);
        ...
        DummyPsyTypeCRecursiveVerifierCircuit {
            proof_a_target, proof_a_verifier_data_target,
            proof_b_target, proof_b_verifier_data_target,
            circuit_data,
        }
    }
    pub fn prove(
        ...
        let mut timing = TimingTree::new("prove recursive (WebGPU)", Level::Trace);
        println!("Starting to prove recursive (WebGPU)...");
        let proof = prove::<F, C, D>(
            &self.circuit_data.prover_only,
            &self.circuit_data.common,
            pw,
...
#[structopt(name = "bench_recursion_webgpu")]
struct Options {
...
fn run_psy_bench_recursion() -> Result<()> {
    ...
    let recursive_proof = dummy_recursive
        .prove(&dummy_proof_1, &dummy_proof_2, &dummy_inner.circuit_data.verifier_only)?;
    ...
    println!("Verifying {} proofs took {:?}, avg time per proof: {:.6} ms", ctr, duration, avg_time * 1000.0);

    println!("WebGPU benchmark recursion proof verified successfully");
    Ok(())
}
...
fn main() -> Result<()> {
    ...
    for threads in threads.clone() {
        rayon::ThreadPoolBuilder::new()
            ...
            .install(|| {
                info!("Using {} compute threads on {} cores", rayon::current_num_threads(), num_cpus);
...
fn parse_range_usize(src: &str) -> Result<RangeInclusive<usize>, ParseIntError> {
    if let Some((left, right)) = src.split_once("..=") {
        ...
        Ok(RangeInclusive::new(
            usize::from_str(left)?,
            if right.is_empty() { usize::MAX } else { usize::from_str(right)?.saturating_sub(1) },
        ))
    } else {
