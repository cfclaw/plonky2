...
/// Convert a standard Goldilocks field element to Montgomery form.
/// Montgomery form: a_mont = a * R mod p, where R = 2^64, p = 2^64 - 2^32 + 1.
/// Since R mod p = 2^32 - 1, we compute a * (2^32 - 1) mod p.
...
/// All 360 Poseidon round constants in standard form (from plonky2).
const ALL_ROUND_CONSTANTS: [u64; 360] = [
...
fn generate_poseidon_wgsl(out_dir: &str) {
    // Read the goldilocks preamble
    let goldilocks_src = include_str!("src/shaders/goldilocks.wgsl");

    // Read the Poseidon shader template (without constants)
    let poseidon_template = include_str!("src/shaders/poseidon_template.wgsl");

    // Generate the round constants array in WGSL
    let mut constants = String::new();
    constants.push_str("// Auto-generated Montgomery-form Poseidon round constants.\n");
    constants.push_str("const RC_MONT: array<vec2<u32>, 360> = array<vec2<u32>, 360>(\n");

    for (i, &c) in ALL_ROUND_CONSTANTS.iter().enumerate() {
        let mont = to_mont(c);
        let lo = mont as u32;
        let hi = (mont >> 32) as u32;
        if i % 4 == 0 {
            constants.push_str("    ");
        }
        constants.push_str(&format!("vec2<u32>(0x{:08x}u, 0x{:08x}u)", lo, hi));
        if i < 359 {
            constants.push_str(", ");
        }
        if i % 4 == 3 {
    ...
    if ALL_ROUND_CONSTANTS.len() % 4 != 0 {
        constants.push('\n');
    }
    constants.push_str(");\n\n");

    // Assemble: goldilocks + constants + poseidon template
    let full_poseidon = format!("{}\n\n{}\n{}", goldilocks_src, constants, poseidon_template);

    let path = Path::new(out_dir).join("poseidon.wgsl");
    std::fs::write(path, full_poseidon).unwrap();
}

fn generate_merkle_wgsl(out_dir: &str) {
    let poseidon_generated = std::fs::read_to_string(Path::new(out_dir).join("poseidon.wgsl")).unwrap();
    let merkle_template = include_str!("src/shaders/merkle_template.wgsl");

    // For merkle, we need the poseidon permute function available.
    // The poseidon.wgsl already has goldilocks + constants + permute.
    // But merkle needs its own bindings/entry points, so we extract just the functions.
    // Simpler approach: include goldilocks + constants + poseidon functions + merkle kernels

    // We'll reuse the poseidon_generated file but strip its entry points (if any),
    // then append merkle template. Since poseidon_template has no entry points, this works.
    let full_merkle = format!("{}\n\n{}", poseidon_generated, merkle_template);

    let path = Path::new(out_dir).join("merkle.wgsl");
    std::fs::write(path, full_merkle).unwrap();
}

fn generate_fft_shaders(out_dir: &str) {
    let goldilocks_src = include_str!("src/shaders/goldilocks.wgsl");

    // Each FFT kernel is a separate shader module
    let kernels = [
        ("fft_coset_scale", include_str!("src/shaders/fft.wgsl")),
        ...
        ("fill_salt", include_str!("src/shaders/fill_salt.wgsl")),
        ("coset_shift_inv", include_str!("src/shaders/coset_shift_inv.wgsl")),
    ];

    for (name, kernel_src) in kernels {
        let full = format!("{}\n\n{}", goldilocks_src, kernel_src);
        let path = Path::new(out_dir).join(format!("{}.wgsl", name));
...
fn main() {
    ...
    // Generate FFT shaders (goldilocks prepended to each)
    generate_fft_shaders(&out_dir);

    // Generate Poseidon shader with Montgomery constants
    generate_poseidon_wgsl(&out_dir);

    // Generate Merkle shader (includes poseidon functions)
    generate_merkle_wgsl(&out_dir);
}
